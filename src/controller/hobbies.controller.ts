...
import { deleteUserHobbies, updateHobbies, findOneUser } from '@src/service/user.service';
...
class HobbiesController implements Controller {
  ...
  delete = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = findOneUser({ _id: userId });
      if (!user) {
        return res.sendStatus(HttpStatusEnum.NOT_FOUND);
      }

      await hobbiesService.deleteHobbies({ _id: hobbiesId });
      const updatedUser = deleteUserHobbies({ _id: userId }, { hobbies: hobbiesId }, { new: true });
